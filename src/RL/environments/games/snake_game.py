...
class SnakeGame:
    ...
    def get_step_info(self):
        ...
        legal_moves = np.ones(shape=(len(self.board.board_directions), ))
        legal_moves[self.board.board_directions[self.board.snake.direction]['INVALID']] = 0

        img = Image.fromarray(obj=board, mode='P')
        img.putpalette(
            data=self.board.background_color + self.board.border.color + self.board.snake.body_color +
            self.board.snake.head_color + self.board.food.color
        )
    ...
    def move_one_step(self, action):
        ...
        h = self.board.snake.coords[0][0] + self.board.board_directions[action]['ADD_TO_COORDS'][0]
        w = self.board.snake.coords[0][1] + self.board.board_directions[action]['ADD_TO_COORDS'][1]
        ...
        if (h, w) in self.board.border.coords:
        ...
        elif (h, w) in self.board.snake.coords[:-1]:
            game_over = True
            self.board.snake.coords.pop(-1)
            self.board.snake.coords.insert(0, (h, w))
            ...
            max_length = ((self.board_size[0]) * (self.board_size[1])) - len(self.board.border.coords)
            if self.board.snake.get_length() == max_length:
                game_over = True
            else:
                game_over = False
